...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 26 13:22:40 2017
...
__all__ = ('TemporalAttachmentLDDMMGeomAtlas', 'RegularityLDDMM',
...
class TemporalAttachmentLDDMMGeomAtlas(Functional):

    """Deformation operator with fixed template acting on displacement fields.



        Compute the attachment term of the deformed template, compared to the
        given data at each time point.

        Compute also the gradient of the attachment term with respect to
        the trajectory of vector field AND initial template

    """


    def __init__(self, nb_time_point_int, data, data_time_points, forward_operators,Norm, kernel, domain=None):
        """
        Parameters
        ----------
        ...
        if domain is None:
            domain = odl.ProductSpace(
                self.image_domain, odl.ProductSpace(
                    self.image_domain.real_space.tangent_bundle,self.N+1))
    ...
    def _call(self, X, out=None):
        template=self.image_domain.element(X[0].copy())
        vector_field_list=X[1].copy()
        ...
        image_integration[0] = self.image_domain.element(template).copy()
    ...
    def gradient(self):
        ...
        class TemporalAttachmentLDDMMGeomAtlasGradient(Operator):
            ...
            def _call(self, X):
                ...
                image_integration = series_image_space_integration.element()
                image_integration[0] = functional.image_domain.element(template).copy()
                image_data = series_image_space_data.element()
...
#                    grad_S_tj=((functional.forward_op[j].adjoint * (functional.forward_op[j] - functional.data[j]))(image_data[j])).copy()
        ...
        return TemporalAttachmentLDDMMGeomAltlasGradient()
    ...
    def gradientL2(self):
        ...
        class TemporalAttachmentLDDMMGeomAtlasGradientL2(Operator):
            ...
            def _call(self, X):
                template=functional.image_domain.element(X[0].copy())
                vector_field_list=X[1].copy()
...
#                    grad_S_tj=((functional.forward_op[j].adjoint * (functional.forward_op[j] - functional.data[j]))(image_data[j])).copy()
                    ...
                    grad[1][functional.k_j_list[j]]-=tmp.copy()
                    ...
                    # Gradient of the template for j-th data
                    grad[0]+=(grad_S[0] * detDphi[0]).copy()


                return grad

        return TemporalAttachmentLDDMMGeomAtlasGradientL2()
