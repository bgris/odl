...
def test_mean_value_difference_sign():
    ...
    I0 = space.one()
    I1 = -I0
    assert fom.mean_value_difference(I0, -I0) == pytest.approx(2.0)
    assert fom.mean_value_difference(I0, I1,
                                     normalized=True) == pytest.approx(2.0)
...
def test_mean_value_difference_range_value(space):
    ...
    assert odl.contrib.fom.mean_value_difference(I0, I1) <= max(max0 - min1,
                                                 max1 - min0)
    assert odl.contrib.fom.mean_value_difference(I0, I0) == pytest.approx(0)
