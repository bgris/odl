...
name_ground_truth_init =  'temporal__t_'
name_list = [ 'temporal__t_' + str(i+ 1 ) for i in range(nb_data_points)]
#num_angles_list = [10, 50, 100, 20]
num_angles_list = [ 10]
#maxiangle_list = ['pi', '0_25pi', '0_5pi', '0_75pi']
#max_angle_list = [np.pi, 0.25*np.pi, 0.5*np.pi, 0.75*np.pi]
...
for num_angles in num_angles_list:
    for maxiangle, max_angle in zip(maxiangle_list, max_angle_list):
            if randompartition:
                # Create random partition and store it
                list_angles = min_angle + (max_angle - min_angle) * np.random.rand(num_angles * len(name_list))
                list_angles = np.sort(list_angles)
                name =  path + name_exp + name_ground_truth_init + 'num_angles_' + str(num_angles) + '_min_angle_' + miniangle + '_max_angle_'
                name += maxiangle + 'randompartition'
                np.savetxt(name + 'angles', list_angles)
                print('random')
            else:
                # Create the uniformly distributed directions
                angle_partition = odl.uniform_partition(min_angle, max_angle, num_angles,
            ...
            for i in range(len(name_list)):
                ...
                for noi, noise_level in zip(noi_list, noise_level_list):
                        ...
                        print(noi)
                        print(noise_level)
                        inter_angle = odl.set.domain.IntervalProd(list_angles[i * num_angles], list_angles[(i+1) * num_angles - 1])
                        grid = odl.discr.grid.RectGrid(list_angles[i * num_angles : (i+1) * num_angles])
